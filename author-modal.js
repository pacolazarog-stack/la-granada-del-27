(()=>{
  let modal=null,lastFocus=null;
  const build=()=>{
    if(modal)return modal;
    modal=document.createElement('div');
    modal.className='author-modal';
    modal.hidden=true;
    modal.setAttribute('role','dialog');
    modal.setAttribute('aria-modal','true');
    modal.setAttribute('aria-label','flag');
    modal.innerHTML=`<div class="author-modal-card" tabindex="-1"><button class="author-modal-close" type="button" aria-label="Cerrar">×</button><div class="author-sign">flag</div><div class="author-fragment"><p><span class="author-twin">Flag</span> quedó perdido sin <span class="author-twin">Fli</span>.</p><p>Y estas palabras,<br>sin ti.</p></div></div>`;
    document.body.appendChild(modal);
    modal.querySelector('.author-modal-close').addEventListener('click',close);
    modal.addEventListener('click',e=>{if(e.target===modal)close();});
    return modal;
  };
  function open(trigger){
    const m=build();
    lastFocus=trigger||document.activeElement;
    m.hidden=false;
    m.querySelector('.author-modal-card').focus({preventScroll:true});
    document.dispatchEvent(new CustomEvent('author:open'));
  }
  function close(){
    if(!modal||modal.hidden)return;
    modal.hidden=true;
    if(lastFocus&&typeof lastFocus.focus==='function')lastFocus.focus({preventScroll:true});
    document.dispatchEvent(new CustomEvent('author:close'));
  }
  document.addEventListener('click',e=>{
    const a=e.target.closest&&e.target.closest('a[href="autor.html"],a[data-author="flag"]');
    if(!a)return;
    e.preventDefault();
    e.stopPropagation();
    open(a);
  },true);
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&modal&&!modal.hidden){e.preventDefault();close();}});
  window.FLAG_AUTHOR={open:()=>open(document.activeElement),close};
})();