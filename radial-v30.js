(()=>{
  ...
  const markInitial=line=>{
    const m=line.match(/\p{L}/u);
    if(!m)return line;
    const i=m.index;
    return line.slice(0,i)+`<span class="rv-initial">${line[i]}</span>`+line.slice(i+1);
  };
  ...
  window.vert=function(){
    ...
    if(pNo===14){
      verses.forEach((z,i)=>{
        ...
        d.innerHTML=`<span class="rv-margin"></span><span class="rv-verse">${markInitial(z)}</span><span class="rv-margin"></span>`;
        host.appendChild(d);
      });
    ...
    verses.forEach((z,i)=>{
      ...
      d.innerHTML=`<span class="rv-margin rv-left">${left}</span><span class="rv-verse">${markInitial(z)}</span><span class="rv-margin rv-right">${right}</span>`;
      host.appendChild(d);
    });

    const panel=document.createElement('div');
    panel.className='rv-panels';
    panel.innerHTML=`<section><div class="rv-label">RAMA SUPERIOR · desde V14 hacia V01</div><strong>${key}</strong><div class="rv-spell">${letters.join(' · ')}</div></section>
                     <section><div class="rv-label">RAMA INFERIOR · desde V14 hacia V27</div><strong>${key}</strong><div class="rv-spell">${letters.join(' · ')}</div></section>`;
