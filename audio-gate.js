(()=>{
  ...
  let unlocked=false,started=false,assetReady=true;
  const audio=new Audio(chosen);
  audio.preload='auto';audio.loop=false;audio.playsInline=true;audio.volume=0.86;
  ...
  const unlockCards=()=>{
    ...
    clock.textContent='CICLO COMPLETADO';fill.style.width='100%';
  ...
  const update=()=>{if(unlocked)return;const d=audio.duration,t=audio.currentTime;if(Number.isFinite(d)&&d>0){fill.style.width=`${Math.min(100,(t/d)*100)}%`;clock.textContent=`${fmt(t)} / ${fmt(d)}`;}};
  const markMissing=()=>{assetReady=false;started=false;start.hidden=false;start.disabled=false;start.textContent='REINTENTAR AUDIO';gate.classList.remove('is-playing');status.textContent='NO SE HA PODIDO CARGAR EL UMBRAL SONORO';clock.textContent='';fill.style.width='0';};
  const begin=async()=>{
    if(!assetReady){assetReady=true;audio.load();}
    try{
      await audio.play();started=true;start.disabled=false;start.hidden=true;gate.classList.add('is-playing');
      status.textContent=pending?`RETORNO DESDE ${origin.toUpperCase()} · ESCUCHE UNA VERSIÓN COMPLETA`:'CICLO SONORO EN CURSO · ACCESO BLOQUEADO';update();
    }catch(_){started=false;start.disabled=false;start.hidden=false;start.textContent=pending?'INICIAR MÚSICA DE RETORNO':'INICIAR CICLO SONORO';gate.classList.remove('is-playing');status.textContent='EL NAVEGADOR REQUIERE UNA ACCIÓN PARA INICIAR EL SONIDO';}
  };

  audio.addEventListener('loadedmetadata',update);
  audio.addEventListener('timeupdate',update);
  audio.addEventListener('playing',()=>{if(!unlocked)start.hidden=true;});
  audio.addEventListener('pause',()=>{if(!unlocked&&started&&!audio.ended){start.disabled=false;start.hidden=false;start.textContent='REANUDAR CICLO';status.textContent='CICLO INTERRUMPIDO · DEBE COMPLETARSE';}});
  audio.addEventListener('ended',async()=>{unlockCards();if(!pending){audio.loop=true;audio.currentTime=0;try{await audio.play()}catch(_){}}});
  audio.addEventListener('error',markMissing);
  start.addEventListener('click',()=>{if(audio.ended||!started)audio.currentTime=0;begin();});

  lockCards();
  status.textContent=pending?`REGRESO DESDE ${origin.toUpperCase()} · PREPARANDO CONTRAPUNTO…`:'INICIANDO UMBRAL SONORO…';
  begin();
})();