...
const source2027=fs.readFileSync('texto-canonico/02_LIBRO_I_2027.md','utf8')
  ...
  .map(cleanLine)
  .filter(Boolean);
const WITHHELD_PUBLIC_LINES=new Set(['[Texto retirado temporalmente de la edición pública.]']);
const unique2027=[...new Set(source2027)].filter(line=>!WITHHELD_PUBLIC_LINES.has(line));
...
const mainPages=['index.html','granada.html','atlas.html','miramar.html','ensayo.html','final.html','autor.html','fli.html'];
