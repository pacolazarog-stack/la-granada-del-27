...
/* La superficie formal y su relación con la piedra se validan en atlas.html.
   LA VEGA de superficie tiene 44 versos visibles: 1+14+14+14+1.
   Su cuerpo estrófico es un tríptico 14+14+14 y umbral/cierre proyectan
   el mismo centro profundo. El texto completo puede permanecer retirado
   de la edición pública sin alterar la matriz 27×27 ni el cubo 27³.
*/
...
const vega=poems[13];
if(vega.kind!=='cyclic-triptych') throw new Error('SUPERFICIE: LA VEGA no está declarada como tríptico cíclico');
if(vega.canonicalVerseCount!==44||vega.visibleVerseCount!==44) throw new Error('SUPERFICIE: LA VEGA debe conservar 44 versos visibles');
if(vega.bodyVerseCount!==42) throw new Error('SUPERFICIE: el cuerpo estrófico de LA VEGA debe conservar 42 versos');
if(JSON.stringify(vega.sectionVerseCounts)!==JSON.stringify([14,14,14])) throw new Error('SUPERFICIE: LA VEGA debe contener tres sonetos de 14 versos');
if(JSON.stringify(vega.centerProjectionIndices)!==JSON.stringify([1,44])) throw new Error('SUPERFICIE: umbral/cierre deben proyectar el centro en 1 y 44');
...
if(!vega.presentation||vega.presentation.canonical!=='three-page-cyclic-triptych'||vega.presentation.pages!==3||vega.presentation.internalPageCounters!==false||vega.presentation.finalMeaningfulSign!=='closure') throw new Error('SUPERFICIE: falta la presentación canónica de tres páginas');
...
console.log(JSON.stringify({
  ...
  surface:{
    ...
    laVega:{
      ...
      visibleVerseCount:vega.visibleVerseCount,
      bodyVerseCount:vega.bodyVerseCount,
      sections:vega.sectionVerseCounts,
      centerProjectionIndices:vega.centerProjectionIndices,
  ...
  cubeCenter:[14,14,14],
  joint:'LA VEGA visible 44 ↔ piedra 27×27 ↔ cubo 27³ = un único centro profundo'
},null,2));
